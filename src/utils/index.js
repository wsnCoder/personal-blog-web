    ...
    for (let i = -25; i <= 25; i += 1) {
      for (let j = -25; j <= 25; j += 1) {
        ...
        if (point.d != -1) {
          ...
          if (a > 0) {
            lets.ctx.fillStyle = colorString(interpolateColors(rgbArray(d / 26 - lets.frameNo / 40), [0, 128, 32], 0.5 + Math.sin(d / 6 - lets.frameNo / 8) / 2))
    ...
    for (let i = -25; i <= 25; i += 1) {
      for (let j = -25; j <= 25; j += 1) {
        ...
        if (point.d != -1) {
          ...
          a = 0.15 - Math.pow(d / 50, 4) * 0.15
          if (a > 0) {
            lets.ctx.fillStyle = colorString(interpolateColors(rgbArray(-d / 26 - lets.frameNo / 40), [32, 0, 128], 0.5 + Math.sin(-d / 6 - lets.frameNo / 8) / 2))
            lets.ctx.globalAlpha = a
            lets.ctx.fillRect(point.x - size / 2, point.y - size / 2, size, size)
          }
        }
  ...
  function draw(lets) {
    ...
    for (let i = 0; i < lets.points.length; ++i) {
      ...
      if (point.d != -1) {
        ...
        if (point.x > -1 && point.x < lets.canvas.width && point.y > -1 && point.y < lets.canvas.height) lets.ctx.fillRect(point.x - size / 2, point.y - size / 2, size, size)
      }
    }
